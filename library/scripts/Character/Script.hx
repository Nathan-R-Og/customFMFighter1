...
self.exports = {
    missiles: self.makeInt(0),
    sideSpeed: self.makeInt(15)
}
...
function update(){
    self.getOwner().setDamage(exports.missiles);//debug
    //works without self. :)
}
...
function sideSpecial(mode){
    //slows based on time spent
    if(mode == 0){
        self.setXSpeed(self.exports.sideSpeed.get());
    }
    else {
        self.setXSpeed(self.exports.sideSpeed / (2.0 * mode));
    }
}